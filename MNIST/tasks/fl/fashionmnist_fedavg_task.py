...
class FashionMNIST_FedAvgTask:
    ...
    def make_optimizer(self, models) -> Optimizer:
        params = []
        for model in models:
        ...
        if self.params.optimizer == "SGD":
            ...
            optimizer = optim.Adam(
                params, lr=self.params.lr, weight_decay=self.params.decay
            )
        else:
    ...
    def get_empty_accumulator(self):
        ...
        for name, data in self.models[0].state_dict().items():
    ...
    def accumulate_weights_weighted(self, weight_accumulator, local_update, weight):
        for name, value in local_update.items():
            weight_accumulator[name].add_(value * weight)
    ...
    def load_data(self) -> None:
        ...
        train_loaders, test_loaders = self.assign_data(
            bias=self.params.fl_q, rotation_angles=self.params.rotation_angles
        )
        self.fl_train_loaders = train_loaders
        self.fl_test_loaders = test_loaders
        return

    def assign_data(self, bias=1, rotation_angles=None, p=0.1):
        ...
        if rotation_angles is None:
            ...
            transform_list = [
                transforms.RandomRotation((0, 0)) for _ in range(num_workers)
            ]
        else:
            logger.info(f"Using rotation angles: {rotation_angles}")
            transform_list = [
                transforms.RandomRotation((rotation_angles[i], rotation_angles[i]))
