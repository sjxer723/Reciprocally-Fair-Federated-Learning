...
class Task:
    ...
    def make_optimizer(self, model=None) -> Optimizer:
        ...
        if self.params.optimizer == "SGD":
            ...
            optimizer = optim.Adam(
                model.parameters(), lr=self.params.lr, weight_decay=self.params.decay
            )
        else:
    ...
    def resume_model(self):
        if self.params.resume_model:
            ...
            loaded_params = torch.load(
                f"saved_models/{self.params.resume_model}",
    ...
    def report_metrics(self, step, prefix="", tb_writer=None, tb_prefix="Metric/"):
        ...
        return self.metrics[0].get_main_metric_value(), self.metrics[
            1
        ].get_main_metric_value()
