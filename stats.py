...
def statistics(path, prefix, name, num_of_clients=30, num_of_types=3):
    ...
    for i in range(1, 1 + num_of_attempts):
        ...
        def accuracy_func_of_i(S, i):
            group_of_i = int(
                i / num_of_clients * num_of_types
            )  # Determine the group of the agent
            denominator = 1
            for j in range(num_of_clients):
                group_of_j = int(
                    j / num_of_clients * num_of_types
                )  # Determine the group of the agent
        ...
        for m in ["br", "br-bg", "br-shap"]:
            ...
            FL = FLInstance(
                num_of_clients, be, 0, 0, _eps=0.1
            )  # alpha and beta are not used here
            shapley_shares = FL.compute_shapley_values(
                lambda clients: accuracy_func(be, clients)
            )
            if m == "br":
                reciprocities = [
                    accuracy_func_of_i(be, i) / shapley_shares[i]
                ...
                payments = [
                    ...
                    * (
                        costs[i]
                        - sum([costs[j] for j in range(num_of_clients) if j != i])
                        / (num_of_clients - 1)
                    )
                    for i in range(num_of_clients)
                ]
                reciprocities = [
                    (accuracy_func_of_i(be, i) + payments[i]) / shapley_shares[i]
                    for i in range(num_of_clients)
                ]
            elif m == "br-shap":
            ...
            all_results[m].append(
                {
                    ...
                    "Welfare": (
                        sum(result_of_m.get("Acc", []))
                        - sum(result_of_m.get("Costs", []))
                    )
                    / num_of_clients,
    ...
    for i in range(num_of_attempts):
        for m in ["br", "br-bg", "br-shap"]:
            if m == "br":
                ...
                all_results[m][i]["DataGain"] = (
                    all_results[m][i]["Data Shares"]
                    * 1.0
                    / all_results["br"][i]["Data Shares"]
                )
                all_results[m][i]["AccGain"] = (
                    all_results[m][i]["Accuracy"]
                    * 1.0
    ...
    for m in ["br", "br-bg", "br-shap"]:
        ...
        for key in all_results[m][0].keys():
            result_for_m[key] = (
                sum([all_results[m][i][key] for i in range(num_of_attempts)])
                / num_of_attempts
            )
        ...
        result_for_m["Method"] = m
        for key in all_results[m][0].keys():
            result_for_m[key] = sorted(
                [all_results[m][i][key] for i in range(num_of_attempts)]
            )[1]
...
info = [
    ...
    (
        ...
        "FashionMNIST_FedAvg_non_iid_fl_results",
        "FashionMNIST",
    ),
