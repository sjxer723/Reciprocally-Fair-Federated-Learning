...
s_eps = (
    50  # each client add s_eps more samples to estimate the gradient of shapley share
)
...
class Client:
    def __init__(
        self, A, B, c, training_size=100, testing_size=50, noise_std=1.0, label=1
    ):
        self.train_X, self.train_y = generate_random_quadratic_data(
            A, B, c, num_points=training_size, delta=1.0, label=label
        )
        self.test_X, self.test_y = generate_random_quadratic_data(
            A, B, c, num_points=testing_size, delta=1.0, label=label
        )
    ...
    for round in range(num_rounds):
        ...
        for i, client in enumerate(clients):
            params, _ = client.train(
                global_model, epochs=local_epochs, lr=lr, share_size=s_vec[i]
            )
...
def fl_run_without_closed_form(
    clients: list[Client], num_rounds=300, local_epochs=5, lr=0.001, verbose=False
):
    # initial data shares
    ...
    for round in range(num_rounds):
        ...
        for i, client in enumerate(clients):
            ...
            params_all.append(params.values())
            params_eps, _ = client.train(
                global_model, epochs=local_epochs, lr=lr, share_size=s_vec[i] + s_eps
            )
            ...
            for i in range(n):
                ...
                with torch.no_grad():
                    for param_g, *param_i in zip(model_fst_i.parameters(), *params_all):
                        ...
                        param_g.copy_(
                            sum([param_i[perm[j]] for j in range(i)]) / (i + 1)
                        )
                # get model T_i^eps
                model_eps_fst_i = copy.deepcopy(global_model)
                with torch.no_grad():
                    for param_g, *param_i in zip(
                        model_eps_fst_i.parameters(), *params_all, *params_eps_all
                    ):
                        param_g.copy_(
                            (
                                (sum([param_i[perm[j]] for j in range(i - 1)]))
                                + param_i[n + perm[i]]
                            )
                            / (i + 1)
                        )
...
def fl_run_with_closed_form(
    ...
    def update_share(S, w, i):
        ...
        if method == "br":
            ...
            s_updated = int(
                S[i] + s_lr * (derivate - (1 - cost_scalar_beta) * costs[i])
            )
        if s_updated >= training_size or s_updated <= 0:
            return S[i]
        else:
...
def _main_for_synthetic_data():
    ...
    for i in range(num_of_labels):
        client = Client(
            A, B, c, training_size=training_size, testing_size=testing_size, label=i
        )
