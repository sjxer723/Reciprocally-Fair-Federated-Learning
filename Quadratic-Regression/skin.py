...
def create_data(X, y, sample_size=10000, test_size=0.2, random_state=42, weights=[0.5, 0.5]):
    np.random.seed(random_state)
    indices_class_0, indices_class_1 = np.where(y == 0)[0], np.where(y == 1)[0]

    indices1 = np.random.choice(indices_class_0, size=int(sample_size * weights[0]), replace=True)
    indices2 = np.random.choice(indices_class_1, size=int(sample_size * weights[1]), replace=True)
...
X_train_B, X_test_B, y_train_B, y_test_B = create_data(X, y, sample_size=max_size, weights=[0.7, 0.3])
...
def run_fl_with_fixed_share(s_vec, _num_rounds=10, verbose=False):
    for round_num in range(_num_rounds):
        ...
        if s1:
            player_A.fit(X_train_A[:s1], y_train_A[:s1], batch_size=16, epochs=epochs_per_round, verbose=0)
        if s2:
            player_B.fit(X_train_B[:s2], y_train_B[:s2], batch_size=16, epochs=epochs_per_round, verbose=0)
        
        new_weights = [(a + b) / 2 for a, b in zip(player_A.get_weights(), player_B.get_weights())]
        ...
        if verbose:
            print(f"Round {round_num + 1} - Player A Accuracy: {acc_A}, Player B Accuracy: {acc_B}")
...
def fit_closed_form_accuracy(fit_sample_delta=10240, n=2, verbose=False):
    ...
    s_vecs = list(itertools.product([fit_sample_delta * i for i in range(num_of_samples_per_dimension + 1)], repeat=n))
...
def best_response(W, costs, method:str, verbose=False):
    ...
    def update_share(S, W, i):
        derivative = derivative_of_accuracy(S, *W, j=i, grouping_fun=lambda x: int((x / n) * num_of_groups))
        s_updated = S[i]
        if method == "br":
            ...
            for perm in perms:
                ...
                derivative_of_shapley += partial_derivative_of_accuracy(S, *W, agents_with_i=agents_with_i, i=i, grouping_fun=lambda x: int((x / n) * num_of_groups))
    ...
    for epoch in range(T):
        ...
        if epoch % 10 == 0 and verbose:
            ...
            print('Epoch: {}, Sum of s_i: {}, Costs: {}'.format(epoch, sum(s_vec), sum(incurred_costs)))
